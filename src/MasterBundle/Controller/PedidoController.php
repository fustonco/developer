...
use MasterBundle\Entity\PedidoAnexo;
use MasterBundle\Entity\Anexo;
use MasterBundle\Entity\Conta;
...
class PedidoController extends Controller
{
    ...
    public function indexContestadoAction(Request $request)
    {
        ...
        $ate = $request->get('ate') ? $request->get('ate') : '';
        $str = "";
        if(($de || $de != "") && ($ate || $ate != "")) {$str = " AND h.data_passagem BETWEEN '".$de." 00:00:01' AND '".$ate." 23:59:59' ";}

        $entities = $em->getConnection()->prepare("
        SELECT sp.id id_status_pedido, e.nome empresa, f.nome funcionario, f.id idFuncionario, fo.nome fornecedor, tu.id id_tipo_funcionario, tu.nome tipo_funcionario, p.id, p.codigo, p.idTipo, p.idFornecedor, p.data_pedido dataPedido, h.data_passagem dataPassagem, p.valor, p.descricao, p.ativo, sp.nome status
        FROM pedido p
        INNER JOIN status_pedido sp ON sp.id = p.status
        INNER JOIN (SELECT MAX(id) id, idPedido FROM historico GROUP BY idPedido) ht ON ht.idPedido = p.id
        INNER JOIN historico h ON ht.id = h.id AND h.idDe = :de
    ...
    public function newAction()
    {
        ...
        $contaTipo = $em->getRepository('MasterBundle:ContaTipo')->findAll();
        $cartao = $em->getRepository('MasterBundle:Cartao')->findBy(['active' => true], [
            'titulo' => 'ASC'
        ]);
    ...
    */
    ...
    public function cadastrarPedidoAction(Request $request)
    {
        ...
        try {
            ...
            $de = $em->getRepository('MasterBundle:Funcionario')->findOneById($this->getUser()->getId());
            $historico->setIdde($de);
            $para = $em->getRepository('MasterBundle:Funcionario')->findOneById($request->para);
            $historico->setIdpara($para);
            $historico->setDataPassagem($hoje);
